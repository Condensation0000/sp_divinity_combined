...
---------------------------
-- This module is intended to be a place where people who enjoy shaman building don't
-- have to choose between playing Divinity (div) ans Spiritual (sp).

-- This module is a combination of sp and div. Players can choose to play sp maps or div
-- maps when they are shaman, and the module will automatically set their shaman mode
-- accordingly. Additionally, players can choose to randomize the possiblility of playing
-- a div or sp map. Any options or exp a player has in sp or div will be kept separate.

-- I am not yet done with this module. The todo

-- Currently, div's owner is missing, and sp is often empty.
...
-- map list for divinity: https://docs.google.com/spreadsheets/d/1cJB5xy-uWkk-D2H2VgQQfXAuBFgmJ3Cu5EugfoIzm9E/edit#gid=1780425394
...
-------------------------------------------------------
-- temporary functions before the module is official --
-------------------------------------------------------
...
-- levels the shaman if applicable
function levelSham()
end

--computes and gives the sham EXP
function scoreSham()
    if not shamScored then
        ...
        -- compute exp
        expGained = scale*(saveBase^numSavedRats)*(diffBase^mapDifficulty)
        expGained = math.floor( expGained - scale*(diffBase^mapDifficulty) )

        -- add the exp
        database['players'][currentSham]['exp'][mapMode] = expGained + database['players'][currentSham]['exp'][mapMode]

        -- level up?
        levelSham()

        -- display info
        tempChatMessage('You gained ' .. expGained .. 'exp.',currentSham)
        tempChatMessage('You now have ' .. database['players'][currentSham]['exp'][mapMode] .. 'exp in ' .. mapMode .. '.',currentSham)
...
-- parses through all the players at the beginnning of a game,
...
function endOfGame()
    ...
    -- reset shams score
    tfm.exec.setPlayerScore(currentSham,0)
...
function eventNewGame()
    ...
    -- grab map info from xml extra tags
    noBloon = currentMapIsNoBloon()
    opportunist = currentMapOpportunist()
...
function eventPlayerWon(playerName)
    print(playerName .. ' has gone in the hole.')
    ...
    -- keep track of how many saves  the sham gets
    numSavedRats = 1+numSavedRats
...
-- this function is called when the shamm starts summoning
function eventSummoningStart(playerName, objectType, xPosition, yPosition, angle)
    if isBloon(objectType) and noBloon then
        tempChatMessage('This is a <font color="#FF0000"><b>NO BALLOON</b></font> map. You cannot summon balloons.',playerName)
        tfm.exec.setShamanMode(currentSham,2)
    end
end


-- the function returns true if the object id is that of a bloon
function isBloon(id)
    str_id = tostring(id)
    start_idx = string.find(str_id,"28")
    return start_idx == 1
end



-- returns true if the map is labeled no bloon in the <P ... \> XML tag
-- false otherwise
function currentMapOpportunist()
    extra_tags = getExtraXMLTags()
    return string.find(extra_tags,"DIVINITYOPPORTUNIST=\"\"") ~= nil and mapMode == 'divinity'
end
...
-- returns true if the map is labeled no bloon in the <P ... \> XML tag
-- false otherwise
function currentMapIsNoBloon()
    extra_tags = getExtraXMLTags()
    return string.find(extra_tags,"DIVINITYNOBALLOON=\"\"") ~= nil and mapMode == 'divinity'
end


-- returns the substring within the <P ... \> XML tag of this current map
function getExtraXMLTags()
    xml = tfm.get.room.xmlMapInfo.xml
    return string.match(xml, "<P (.-)/>")
end


-- stop tfm's auto BS
-- tfm.exec.disableAfkDeath(true) -- TODO: implement 'smart' afk death
